        ...
        for (int j = 0; j < tc; j++)
        {
            ...
            for (int k = 0; k < current_br; k++)
            {
                for (int l = 0; l < current_bc; l++)
                {
                    ...
                    for (int m = 0; m < d; m++)
                    {
                        float a = Q_tile[k * d + m];
                        float b = Kt_tile[m * current_bc + l];
                        FakeUse(a);
                        FakeUse(b);
                    }
                    S[k * current_bc + l] = tmp;
                    FakeUse(S[k * current_bc + l]);
                }
            }
            auto t3 = std::chrono::steady_clock::now();
            seg1 += std::chrono::duration<double, std::milli>(t3 - t2).count();
            ...
            for (int k = 0; k < current_br; k++)
            {
                ...
                for (int j = 0; j < current_bc; j++)
                {
                    float v = S[k * current_bc + j];
                    FakeUse(v);
                    // row_sum(S.data() + k * current_bc, current_bc)
                }
                v = M_i[k];
                float u = Mi_new[k];
                float m = L_i[k];
                ...
                seg2 += std::chrono::duration<double, std::milli>(t5 - t4).count();
            ...
            for (int k = 0; k < current_br; k++)
            {
                ...
                for (int l = 0; l < d; l++)
                {
                    ...
                    for (int m = 0; m < current_bc; m++)
                    {
                        ...
                        float v = S[k * current_bc + m];
                        float u = V_tile[m * d + l];
                        FakeUse(v);
                        FakeUse(u);
            ...
            seg3 += std::chrono::duration<double, std::milli>(t7 - t6).count();
        }
    }
